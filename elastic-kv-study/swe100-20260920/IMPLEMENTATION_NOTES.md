...
- `continuum-paper`：按论文式(2)补建CDF TTL。`T`用最近128次抢占后重入队、或返回时实际APC命中低于上一轮可复用LCP的请求等待时间均值近似；eta仅使用已经结束的job统计，并裁剪到[0,1]。新工具没有历史时使用公开代码的两秒冷启动值。这些是实现选择，不声称恢复了作者未公开细节。
